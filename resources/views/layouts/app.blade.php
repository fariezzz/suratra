...
<head>
    ...
    <title>{{ $title ?? 'Sistem Informasi Persuratan RT/RW' }}</title>
            ...
            <div class="collapse navbar-collapse" id="mainMenu">
                <ul class="navbar-nav ms-auto align-items-lg-center gap-lg-2">
                    ...
                    @if (auth()->user()->isRt() || auth()->user()->isRw())
                        <li class="nav-item">
                            <a class="nav-link @if (request()->routeIs('residents.*')) active @endif" href="{{ route('residents.index') }}">
                                <i class="bi bi-people me-1"></i>Data Warga
                            </a>
                        </li>
                    @endif
                    <li class="nav-item">
                        <a class="nav-link @if (request()->routeIs('letters.*')) active @endif" href="{{ route('letters.index') }}">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li><span class="dropdown-item-text text-muted">{{ auth()->user()->role?->label() ?? 'Pengguna' }}</span></li>
